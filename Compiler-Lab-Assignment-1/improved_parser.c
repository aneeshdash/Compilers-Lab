...
statements()
{
    /*  statements -> id = expression SEMI |  if expression then statements SEMI | while expression do statements SEMI|
    begin opt_statements SEMI end */
    ...
    while( !match(EOI) )
    {
        ...
        else if ( match (IF) )
        {
            ...
            {
                fprintf( stderr, "%d: No then block following if\n", yylineno );
            }
        }
